...
const StagingArea = ({ onLogout, user }) => {
    ...
    const [formTypeFilter, setFormTypeFilter] = useState('全部');
        ...
        //當選中超過一個PDF，必須先合併，才能更新後端寄件PDF名稱
            ...
            try {
                ...
                if (data.success) {
                    ...
                    const returnedFormType = data.formType; // 從後端回來的 formType
            ...
            <div style={{marginLeft: '30px',marginRight: '30px'}}>
                ...
                {forms.length > 0 && (
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap',marginTop:'10px' }}>
                        {availableFormTypes && availableFormTypes.map((type) => (
                            <button
                                key={type}
                ...
                {isLoading ? (
                    ...
                    .map((form, formIndex) => (
                        <li
                            key={form.form_id}
                            className={formIndex % 2 === 0 ? "dark-bg" : "light-bg"}
                            style={{
                                display: 'flex',
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <input
                                    ...
                                    onChange={(event) => handleCheckboxChange(event, form.pdf_name)}
                                    style={{
                                        width: '20px',
                                        height: '20px',
                                        marginRight: '10px',
                                        appearance: 'none',
                                        backgroundImage: `url(${selectedForms.includes(form.pdf_name) ? TickYellow : ''})`,
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                ...
                                <button
                                    onClick={() => handleDelete(form.form_id, form.pdf_name, form.form_type)}
                                    className={`previewdelete-button ${selectedForms.includes(form.pdf_name) ? 'selected' : 'default'}`}
                                >
                                    刪除
                                </button>
